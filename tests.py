...
class TestIRC(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_nick_join_privmsg_quit(self):
        ...
        self.assertIsInstance(joe.state, aioircd.UserRegisteredState, "Joe should be registered")
        self.assertFalse(joe_read.all_messages(), "The NICK command should't send a reply")     

        leo_write.write(b'NICK Leo\r\n')
        await leo_write.drain()
        leo = await self.wait(lambda: server.local.users.get('Leo'))
        self.assertIsInstance(leo.state, aioircd.UserRegisteredState, "Leo should be registered")
        self.assertFalse(leo_read.all_messages(), "The NICK command should't send a reply")
